...
def gibbs_helper(spec,T):
    ...
    for i in range(0,len(lines)):
        if lines[i][0:len(spec)]==spec:
            j+=1
            lines_spec = lines[i:i+15]
            for ii in range(0,len(lines_spec)):
                if lines_spec[ii][0:7]=='    200':
                    line_1 = lines_spec[ii+1].replace('D','E').replace('\n','')
                    line_2 = lines_spec[ii+2].replace('D','E').replace('\n','')
...
def henrys_coef(key,T,P):
    ...
    H = np.exp((gibbsG(key+'  ',T)-gibbsAQ(key,T,P))/(R*T))
    return H
