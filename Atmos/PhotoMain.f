...
C      PHICH4 = - DK(LCH4,1)*(USOL(LCH4,2) - USOL(LCH4,1))/DZ
C     2  - 0.5*(HI(LCH4,1)*DEN(1)*USOL(LCH4,1) + HI(LCH4,2)*DEN(2)
C     3         *USOL(LCH4,2)) - (YP(LCH4,1) - YL(LCH4,1)*SL(LCH4,1))*DZ
C      H2DEP = VDEP(LH2)*SL(LH2,1)
C      H2VOLC = 2.E10 + 0.04*(0.5*ABS(H2DEP) + 0.1*PHICH4)
...
C Below is Nick's test of having distributed fluxes and
C fixed mixing ratios
C      do i=1,nq
C       if (LBOUND(i).eq.1) then
C     convert to cm
C        disth=distheight(i)*1.e5
C     height index (-1 given the staggered grid)
C     the 1 in the second postion tells minloc to return a scalar
C        jdisth=minloc(Z,1, Z .ge. disth)-1
C
C
C        ZTOP=Z(jdisth)-Z(1)
C        ZTOP1=Z(jdisth)+0.5*DZ(jdistH)
c        print *, ISPEC(i),distH,jdistH,ZTOP,ZTOP1

C     distribute from second level to distheight
C        do j=2,jdisth
C          K = i + (J-1)*NQ
C          rhs(k) = rhs(k) + 2.*SGFLUX(i)*(ZTOP1-Z(j))/(Den(j)*ZTOP**2)
C        enddo
C       endif
C      enddo
