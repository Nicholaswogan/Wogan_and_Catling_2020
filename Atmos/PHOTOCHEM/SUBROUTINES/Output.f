...
C     Eco model stuff
C      T = 298.
C     Use DG = 30 since that is energy needed to make 1 ATP
      DELTA_G = -30.
...
C     Calculate [CH4]aq assuming piston velocity formulation for CH4 flux:
C     PHI_CH4 = VPIS_CH4 * SCALE * ([CH4]aq - ALPHA_CH4*pCH4)
      CH4_aq = FLOW(LCH4)/(VPIS_CH4*SCALE) + EQUIL_CH4

      ALPHA_H2 = 7.8E-4
      EQUIL_H2 = ALPHA_H2*USOL(LH2,1)
C     [H2]aq calculated from Q = [CH4]aq*aCO2/([CO2]aq*aCH4 * ([H2]aq*aH2)^4)
      H2_aq = ALPHA_H2*(CH4_aq*ALPHA_CO2/(CO2_aq*ALPHA_CH4*Q))**0.25

      PRINT 169
 169  FORMAT(1X,'DISSOLVED CH4 AND H2 (ECO MODEL)'/)
      CH4_H2_RATIO = USOL(LCH4,1)/USOL(LH2,1)
C     See main program (deposition velocities section) for derivation of vmax(H2)
      VMAX_H2 = 2.4E-4
      VECO_H2 = VMAX_H2*(EQUIL_H2 - H2_aq)/EQUIL_H2
      VPHOTO_H2 = ABS(FLOW(LH2)/(USOL(LH2,1)*DEN(1)))
      VRATIO = VPHOTO_H2/VECO_H2
      PRINT 168, VPIS_CH4,CH4_aq,EQUIL_CH4,H2_aq,EQUIL_H2,CH4_H2_RATIO,
     2           VMAX_H2,VPHOTO_H2,VECO_H2,VRATIO
 168  FORMAT(5X,'VPIS(CH4) =',1PE10.3,3X,'[CH4]aq =',1PE11.4,
     2  3X,'alphaCH4*pCH4 =',1PE11.4,3X,'[H2]aq =',1PE11.4,
     3  3X,'alphaH2*pH2 =',1PE11.4,//5X,'f(CH4)/f(H2) =',1PE11.4,
     4  3X,'VMAX(H2) =',1PE11.4,3X,'VPHOTO(H2) =',1PE11.4,
     5  3X,'VECO(H2) =',1PE11.4,3X,'VPHOTO(H2):VECO(H2) =',1PE11.4/)

!    Try to write eco things to out.out
      write(14, 167)
 167  FORMAT(1X,'DISSOLVED CH4 AND H2 (ECO MODEL)'/)
      write(14, 166) VPIS_CH4,CH4_aq,EQUIL_CH4,H2_aq,EQUIL_H2
      write(14, 165) CH4_H2_RATIO,VMAX_H2,VPHOTO_H2,VECO_H2,VRATIO
 166  FORMAT(5X,'VPIS(CH4) =',1PE10.3,3X,'[CH4]aq =',1PE11.4,
     2  3X,'alphaCH4*pCH4 =',1PE11.4,3X,'[H2]aq =',1PE11.4,
     3  3X,'alphaH2*pH2 =',1PE11.4)
 165  FORMAT(5X,'f(CH4)/f(H2) =',1PE10.3,3X,'VMAX(H2) =',1PE11.4,
     2  3X,'VPHOTO(H2) =',1PE11.4,3X,'VECO(H2) =',1PE11.4,
     3  3X,'VPHOTO(H2):VECO(H2) =',1PE11.4)
...
!I have updated the sulfur balance, so will probably be able to follow the same scheme.
 ...
 667  format(/,'redox conservation = ',1PE10.3,
     2  ' mol/cm^2/s, a factor of ',1PE10.3, ' NEW METHOD' /)
      print 668 ,(0.5*FUP(4)+FUP(8))
 668  format(/,'Hydrogen escape = ',1PE10.3/)
